...
{
    Mat luminance;
    cvtColor(frame, luminance, cv::COLOR_BGR2GRAY);//CV_BGR2GRAY
    // Add scratches
    ...
    cvtColor(hsvFrame, retroFrame, cv::COLOR_HSV2RGB);//CV_HSV2RGB
}
...
void RetroFilter::applyToVideo(const Mat& frame, Mat& retroFrame)
{
    ...
    cvtColor(frame(roiSrc), retroFrame(roiDst), cv::COLOR_BGR2GRAY);//CV_BGR2GRAY
    ...
    cvtColor(retroFrame, retroFrame, cv::COLOR_GRAY2BGR);//CV_GRAY2BGR
}
